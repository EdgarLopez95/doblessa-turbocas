...
| **BUG-03** | `src/components/HelpfulDataPanel.astro`, `ServiceRequest.astro`, `contacto.astro` | Los usuarios no sabían qué datos o papeles preparar antes de llamar o consultar por formulario, y el panel carecía de la 5ª viñeta y del cierre requerido. | **Resuelto** | Diseñado e insertado panel visible con las 5 viñetas exactas (vehículo, referencia en placa, síntomas/testigos, foto y aviso previo de expedición fuera de Castellón) y cierre exacto: *«Con lo que tengas a mano empezamos.»* (Commits `8a610ae` y `e5d201d`). |
...
| **BUG-12** | `scripts/verify-build.mjs`, `package.json` | Ausencia de verificación de calidad automatizada integrada en el repositorio. | **Resuelto** | Creado script `scripts/verify-build.mjs` con comando `npm run verify` que audita `dist/` (13 HTML, 4 CSS, 1 H1 por página, 0 `href="#"`, 0 Google Fonts, `noindex` en todas las páginas, 0 `info@turbocas.com`, presencia obligatoria del cierre del panel de datos y fuentes locales). (Commit `34f3450`). |
| **BUG-13** | `src/pages/politica-de-privacidad.astro` | Texto legal desactualizado afirmaba que el sitio cargaba tipografías desde Google Fonts, cuando ya estaban autoalojadas en local. | **Resuelto** | Sustituido por el texto exacto requerido: *«Las tipografías de esta propuesta se sirven desde el propio sitio. Los formularios son demostrativos: no envían ni almacenan datos. El correo del responsable de privacidad está pendiente de confirmar por Turbocas.»* (Commit `e5d201d`). |
...
7. **Privacidad y recursos locales**: Cero peticiones externas hacia Google Fonts o dominios de terceros.
